...
DOCKER_RUN_GO_RELEASER := @docker run \
	...
	--rm \
	--volume `pwd`:/go/src/go-icq \
	--workdir /go/src/go-icq \
...
.PHONY: docker-run
docker-run:
	OSCAR_HOST=$(OSCAR_HOST) docker compose up go-icq stunnel

.PHONY: docker-run-bg
docker-run-bg: ## Run Open OSCAR Server in background with docker-compose
	OSCAR_HOST=$(OSCAR_HOST) docker compose up -d go-icq stunnel
