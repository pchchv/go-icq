...
// RequestRoomInfo returns wire.ChatNavNavInfo,
// which contains metadata for the chat room specified in the inFrame.hmacCookie.
func (s ChatNavService) RequestRoomInfo(ctx context.Context, inFrame wire.SNACFrame, inBody wire.SNAC_0x0D_0x04_ChatNavRequestRoomInfo) (wire.SNACMessage, error) {
	...
	} else if room.Exchange() != inBody.Exchange {
