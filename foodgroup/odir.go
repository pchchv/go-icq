...
// email, or interest.
// It dispatches the request to the appropriate search method and returns the search results or an error.
// The search type is determined by the presence of certain TLVs:
//   - wire.ODirTLVEmailAddress: Search by email.
//   - wire.ODirTLVInterest: Search by interest keyword.
//   - wire.ODirTLVFirstName or wire.ODirTLVLastName:
//     Search by name and address.
//     First name or last name must be required to search by name and address.
//
// AIM 5.x sends wire.ODirTLVSearchType to specify the search type.
// This TLV is ignored in order to be backwards compatible with older versions that do not send it.
// It doesn't appear to make a difference,
// since AIM 5.x sends the same TLV types for each search type.
...
// searchResponse constructs the SNAC reply based on the users found during the search.
