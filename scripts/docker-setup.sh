...
# docker-setup.sh
# Cross-platform installer for Go ICQ Docker Setup
...
resolve_repo_root() {
    ...
    if git -C "$SCRIPT_DIR" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
        ...
        if [ -d "$TMP_DIR/.git" ]; then
            ...
            log "Cloning Go ICQ repository..."
            if ! git clone https://github.com/mk6i/go-icq.git "$TMP_DIR"; then
...
start_server() {
    log "Starting Go ICQ Server with hostname $OSCAR_HOST..."
    if ! make docker-run-bg OSCAR_HOST="$OSCAR_HOST"; then
        error "Failed to start server"
        return 1
    fi
    success "Go ICQ Server is running."
}

final_steps() {
    printf "%b\n" "${GREEN}
Go ICQ Server setup complete!
===================================
Next Steps:
...
${GREEN}3. For AIM 6.x client setup instructions, see:
   https://github.com/mk6i/go-icq/blob/main/docs/AIM_6_7.md#aim-62-75-setup
