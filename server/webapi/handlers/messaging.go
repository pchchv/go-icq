...
func (h *MessagingHandler) SendIM(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		switch err {
		case state.ErrNoWebAPISession, state.ErrWebAPISessionExpired:
			h.sendErrorResponse(w, http.StatusUnauthorized, "invalid or expired session")
		default:
			h.sendErrorResponse(w, http.StatusInternalServerError, "internal server error")
		}
	...
	if recipientSession == nil {
		...
		h.Logger.DebugContext(ctx, "queued sentIM event for sender", "from", sess.ScreenName.String(), "to", recipient, "eventType", types.EventTypeSentIM)
		h.Logger.DebugContext(ctx, "delivered instant message", "from", sess.ScreenName.String(), "to", recipient)
	}
