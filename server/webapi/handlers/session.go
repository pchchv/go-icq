...
func (h *SessionHandler) StartSession(w http.ResponseWriter, r *http.Request) {
	...
	if eventsParam != "" {
		...
		h.Logger.DebugContext(ctx, "parsing events from request", "eventsParam", eventsParam, "parsedEvents", events)
	} else {
		// default events if none specified
		events = []string{"buddylist", "presence", "im", "sentIM"}
		h.Logger.DebugContext(ctx, "using default events", "events", events)
	}
	...
	if authToken != "" {
		...
		// for WebAPI sessions,
		// can use the IdentScreenName directly as DisplayScreenName since WRAITH handles the display formatting
		...
		h.Logger.Info("authenticated session requested", "token", tokenPreview, "screenName", screenName)
	} else {
		// anonymous session
		// generate guest name
		screenName = state.DisplayScreenName("Guest_" + strconv.FormatInt(time.Now().Unix(), 36))
		h.Logger.Info("anonymous session requested", "screenName", screenName)
	}
	...
	h.Logger.DebugContext(ctx, "session created with event subscriptions", "aimsid", session.AimSID, "events", events)
	// store client info
	...
	// if buddy list event is subscribed,
	// include initial buddy list
	for _, event := range events {
		if event == "buddylist" {
